# In this file we apply our all previusly four files to get the output from the image pass in the function. We actually trained
# our model to get the required output from the unseen input
...
def infer(model: Model, fn_img: Path) -> None:
    ...
    return recognized[0]  # type: ignore
...
def infermain():
    ...
    if args.mode in ['train', 'validate']:
        ...
        if args.mode == 'train':
            model = Model(char_list, decoder_type)  # type: ignore
            train(model, loader, line_mode=args.line_mode, early_stopping=args.early_stopping)
        elif args.mode == 'validate':
            model = Model(char_list, decoder_type, must_restore=True)  # type: ignore
            validate(model, loader, args.line_mode)

    # infer text on test image
    elif args.mode == 'infer':
        model = Model(list(open(FilePaths.fn_char_list).read()), decoder_type, must_restore=True, dump=args.dump)  # type: ignore
