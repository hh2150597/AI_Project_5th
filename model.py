# In this file we train our model using cnn rnn and ctc. Also we set up tensor flow to save the model if there is already savedd model we 
# use it. Also we write some function to recognize text from the image provided as an input.
...
import tensorflow as tf # Tensorflow is an open-source library for numerical computation
from typing import List, Tuple # used to store multiple items in a single variable. # it lets you specify a specific number of elements expected and the type of each position
from dataloader_iam import Batch # from previous dataloader_iam file import batch

# Disable eager mode
tf.compat.v1.disable_eager_execution() # Eager execution is a powerful execution environment that evaluates operations immediately
...
class Model:
    ...
    def __init__(self,
                 char_list: List[str],
                 decoder_type: str = DecoderType.BestPath, # type: ignore
    ...
    def setup_rnn(self) -> None:
        ...
        # bidirectional RNN
        # BxTxF -> BxTx2H
        (fw, bw), _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=stacked, cell_bw=stacked, inputs=rnn_in3d, dtype=rnn_in3d.dtype)
    ...
    def setup_ctc(self) -> None:
        ...
        self.loss = tf.reduce_mean(
            input_tensor=tf.compat.v1.nn.ctc_loss(labels=self.gt_texts, inputs=self.ctc_in_3d_tbc, sequence_length=self.seq_len, ctc_merge_repeated=True))
    ...
    def train_batch(self, batch: Batch) -> float:
        ...
        feed_dict = {self.input_imgs: batch.imgs, self.gt_texts: sparse, self.seq_len: [max_text_len] * num_batch_elements, self.is_train: True}
        _, loss_val = self.sess.run(eval_list, feed_dict) # type: ignore
    ...
    def infer_batch(self, batch: Batch, calc_probability: bool = False, probability_of_gt: bool = False):
        ...
        if self.decoder_type != DecoderType.WordBeamSearch:
            decoded = eval_res[0] # type: ignore
        # word beam search decoder: decoding is done in C++ function compute()
        else:
            decoded = self.decoder.compute(eval_res[0]) # type: ignore
        ...
        if calc_probability:
            ...
            ctc_input = eval_res[1] # type: ignore
            ...
            probs = np.exp(-loss_vals) # type: ignore

        # dump the output of the NN to CSV file(s)
        if self.dump:
            self.dump_nn_output(eval_res[1]) # type: ignore
