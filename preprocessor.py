# In this file we make an input file. Actually create an image of a text line by pasting multiple word images into an image
# and then we process over the created image to resize it to target size by apply data augementation (creating new data points
# from alrdy exits data) and then we compute ctc loss.
...
class Preprocessor:
    ...
    def process_img(self, img: np.ndarray) -> np.ndarray:
        ...
        if self.data_augmentation:
            ...
            M = np.float32([[fx, 0, tx], [0, fy, ty]]) # type: ignore
            target = np.ones(self.img_size[::-1]) * 255
            img = cv2.warpAffine(img, M, dsize=self.img_size, dst=target, borderMode=cv2.BORDER_TRANSPARENT) # type: ignore
        ...
        # no data augmentation
            ...
            # map image into target image
            M = np.float32([[f, 0, tx], [0, f, ty]]) # type: ignore            
            target = np.ones([ht, wt]) * 255
            img = cv2.warpAffine(img, M, dsize=(wt, ht), dst=target, borderMode=cv2.BORDER_TRANSPARENT) # type: ignore
        # transpose for TF
        img = cv2.transpose(img)

        # convert to range [-1, 1]
        img = img / 255 - 0.5 # type: ignore        
        return img

    def process_batch(self, batch: Batch) -> Batch:
        """Process over batch tuple"""
        if self.line_mode:
            batch = self._simulate_text_line(batch) # function defined above

        res_imgs = [self.process_img(img) for img in batch.imgs] # process over all  images
        max_text_len = res_imgs[0].shape[0] // 4
        res_gt_texts = [self._truncate_label(gt_text, max_text_len) for gt_text in batch.gt_texts] # function defined above
        return Batch(res_imgs, res_gt_texts, batch.batch_size)
